...
class Server extends AJAXServer {
    ...
    public function handleAction( $request , $filePaths) {
        ...
        if($this->validateUser($request['userid'], $usersPath)){                // Check if username is valid 
            $files = scandir($levelPath);                                       // Get the filenames of the objects in directory
            ...
            $array = array("payload" =>$payload , "error" => 0);    // Create combine error message and payload data
            $response = $array;                               // Convert array to JSON and print it
        
        }
        else{
            $array = array("error" => 1);                           // Set error
            $response = $array;                               // Convert array to JSON and print it
        }
        return $response;
    }
